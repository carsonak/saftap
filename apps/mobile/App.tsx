...
const ActivityItem = ({ title, sub, amount, kes, type }: any) => (
  <View style={styles.listItem}>
    <View
      style={[
        styles.listIconBox,
        { backgroundColor: type === "in" ? "rgba(58, 223, 171, 0.1)" : "rgba(248, 113, 113, 0.1)" },
      ]}
    >
      {type === "in" ? (
        <ArrowDownLeft color={theme.colors.primary} size={20} />
      ) : (
        <ArrowUpRight color={theme.colors.accentRed} size={20} />
      )}
    </View>
    <View style={{ flex: 1 }}>
    ...
    <View style={{ alignItems: "flex-end" }}>
      <Text style={[styles.listAmount, { color: type === "in" ? theme.colors.primary : "#F87171" }]}>
        {amount}
      </Text>
      <Text style={styles.listKes}>{kes}</Text>
    </View>
  </View>
);
...
// ─── Shared hook for fonts + ready state ─────────────────────────────────────
...
function useAppFonts() {
  ...
  return { isReady, fontsLoaded, getFont };
}

// ─── Home Screen ─────────────────────────────────────────────────────────────

function HomeScreen({ navigation }: any) {
  const { isReady, getFont } = useAppFonts();
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollPadding}>
        ...
        <View style={styles.balanceCard}>
          <View style={styles.balanceHeaderRow}>
            <Text style={[styles.labelCaps, { fontFamily: getFont("DMSans_700Bold") }]}>
              TOTAL BALANCE
            </Text>
            <View style={styles.mainWalletBadge}>
              <View style={styles.greenDot} />
              <Text style={[styles.badgeTextSmall, { fontFamily: getFont("DMSans_700Bold") }]}>
                Main Wallet
              </Text>
            </View>
          </View>
          <View style={styles.balanceContent}>
            <Text style={[styles.balanceValue, { fontFamily: getFont("Syne_700Bold") }]}>
              $2,480.50 <Text style={styles.currencySub}>USDC</Text>
            </Text>
            <Text style={[styles.kesValue, { fontFamily: getFont("JetBrainsMono_500Medium") }]}>
              ≈ KES 319,584
            </Text>
          </View>
          <TouchableOpacity style={styles.fundButton}>
            <Text style={[styles.fundButtonText, { fontFamily: getFont("DMSans_700Bold") }]}>
              FUND WALLET
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.actionGrid}>
          <QuickAction
            label="Payment"
            icon={<Smartphone color={theme.colors.primary} size={24} />}
          ...
          <QuickAction
            label="Send"
            icon={<SendIcon color={theme.colors.primary} size={24} />}
            onPress={() => navigation.navigate("Payment")}
          />
          <QuickAction
            label="Swap"
            icon={<ArrowLeftRight color={theme.colors.primary} size={24} />}
          />
        </View>
        ...
        <ActivityItem
          title="Sent to Aisha M."
          sub="Via M-Pesa · 2h ago"
          amount="-$50.00"
          kes="KES 6,450"
          type="out"
        />
        ...
        <View style={styles.offerCard}>
          <View style={styles.offerContent}>
            <Text style={[styles.offerTitle, { fontFamily: getFont("Syne_700Bold") }]}>
              Travel Insurance
            </Text>
            <Text style={[styles.offerSub, { fontFamily: getFont("DMSans_400Regular") }]}>
              Secure your trip to Nairobi with instant coverage in USDC.
            </Text>
          </View>
...
function VisaScreen() {
  ...
  if (!isReady) return <View style={[styles.container, styles.centered]}><ActivityIndicator color={theme.colors.primary} size="large" /></View>;
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollPadding}>
        <View style={styles.header}>
          ...
          <Text style={[styles.brandText, { fontFamily: getFont("Syne_700Bold") }]}>SafTap</Text>
          <TouchableOpacity>
            <MoreVertical color={theme.colors.textSecondary} size={24} />
          </TouchableOpacity>
        </View>
        ...
        <LinearGradient colors={["#1F2937", "#111827"]} style={styles.virtualCard}>
          <View style={styles.cardLogoRow}>
            ...
            <Text style={[styles.cardType, { fontFamily: getFont("DMSans_700Bold") }]}>VIRTUAL DEBIT</Text>
            <View style={styles.visaIconPlaceholder}>
          ...
          <View style={styles.cardBottomRow}>
            ...
            <TouchableOpacity style={styles.nfcIcon}>
              <Smartphone color={theme.colors.primary} size={18} />
            </TouchableOpacity>
          </View>
        </LinearGradient>
        ...
        <View style={styles.cardActionRow}>
          <TouchableOpacity style={styles.cardActionButton}>
            <Snowflake color={theme.colors.primary} size={20} />
            <Text style={[styles.cardActionText, { fontFamily: getFont("DMSans_700Bold") }]}>Freeze Card</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.cardActionButton}>
            <Eye color={theme.colors.primary} size={20} />
            <Text style={[styles.cardActionText, { fontFamily: getFont("DMSans_700Bold") }]}>View PIN</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.limitContainer}>
          <View style={styles.limitHeader}>
            ...
            <Text style={[styles.limitValues, { fontFamily: getFont("JetBrainsMono_500Medium") }]}>
              <Text style={{ color: theme.colors.primary }}>$4,250</Text> / $5,000
            </Text>
          </View>
        ...
        <View style={styles.settingsGroup}>
          <View style={styles.settingItem}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.settingTitle, { fontFamily: getFont("DMSans_700Bold") }]}>International Payments</Text>
              <Text style={styles.settingSub}>Enable for foreign transactions</Text>
            </View>
          ...
          <View style={styles.settingItem}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.settingTitle, { fontFamily: getFont("DMSans_700Bold") }]}>Online Purchases</Text>
              <Text style={styles.settingSub}>Allow web and in-app payments</Text>
            </View>
...
function PaymentScreen() {
  ...
  if (subScreen === "mpesa_send") return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.scrollPadding}>
          <View style={styles.header}>
            <TouchableOpacity onPress={() => setSubScreen("main")} style={styles.backButton}>
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.inputBox}>
              <TextInput style={[styles.formInput, { fontFamily: getFont("JetBrainsMono_500Medium") }]} placeholder="e.g. 0712345678" placeholderTextColor={theme.colors.textSecondary} keyboardType="phone-pad" value={phoneNumber} onChangeText={setPhoneNumber} />
            </View>
            <Text style={[styles.label, { fontFamily: getFont("DMSans_400Regular"), marginTop: 24 }]}>Amount (KES)</Text>
            <View style={styles.inputBox}>
              <TextInput style={[styles.formInput, { fontFamily: getFont("JetBrainsMono_500Medium") }]} placeholder="0.00" placeholderTextColor={theme.colors.textSecondary} keyboardType="numeric" value={amount} onChangeText={setAmount} />
            </View>
            <TouchableOpacity style={[styles.primaryButton, (!phoneNumber || !amount) && styles.buttonDisabled]} disabled={!phoneNumber || !amount || isProcessing} onPress={handlePayment}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.scrollPadding}>
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.inputBox}>
              <TextInput style={[styles.formInput, { fontFamily: getFont("JetBrainsMono_500Medium") }]} placeholder="e.g. 888888" placeholderTextColor={theme.colors.textSecondary} keyboardType="numeric" value={bizNumber} onChangeText={setBizNumber} />
            </View>
            <Text style={[styles.label, { fontFamily: getFont("DMSans_400Regular"), marginTop: 24 }]}>Account Number</Text>
            <View style={styles.inputBox}>
              <TextInput style={[styles.formInput, { fontFamily: getFont("JetBrainsMono_500Medium") }]} placeholder="e.g. ACC-123" placeholderTextColor={theme.colors.textSecondary} value={accountNumber} onChangeText={setAccountNumber} />
            </View>
            <Text style={[styles.label, { fontFamily: getFont("DMSans_400Regular"), marginTop: 24 }]}>Amount (KES)</Text>
            <View style={styles.inputBox}>
              <TextInput style={[styles.formInput, { fontFamily: getFont("JetBrainsMono_500Medium") }]} placeholder="0.00" placeholderTextColor={theme.colors.textSecondary} keyboardType="numeric" value={amount} onChangeText={setAmount} />
            </View>
            <TouchableOpacity style={[styles.primaryButton, (!bizNumber || !accountNumber || !amount) && styles.buttonDisabled]} disabled={!bizNumber || !accountNumber || !amount || isProcessing} onPress={handlePayment}>
              {isProcessing ? <ActivityIndicator color={theme.colors.onPrimary} /> : <Text style={[styles.primaryButtonText, { fontFamily: getFont("DMSans_700Bold") }]}>Make Payment</Text>}
            </TouchableOpacity>
          </View>
        </ScrollView>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.scrollPadding}>
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.inputBox}>
              <TextInput style={[styles.formInput, { fontFamily: getFont("JetBrainsMono_500Medium") }]} placeholder="e.g. 123456" placeholderTextColor={theme.colors.textSecondary} keyboardType="numeric" value={tillNumber} onChangeText={setTillNumber} />
            </View>
            <Text style={[styles.label, { fontFamily: getFont("DMSans_400Regular"), marginTop: 24 }]}>Amount (KES)</Text>
            <View style={styles.inputBox}>
              <TextInput style={[styles.formInput, { fontFamily: getFont("JetBrainsMono_500Medium") }]} placeholder="0.00" placeholderTextColor={theme.colors.textSecondary} keyboardType="numeric" value={amount} onChangeText={setAmount} />
            </View>
            <TouchableOpacity style={[styles.primaryButton, (!tillNumber || !amount) && styles.buttonDisabled]} disabled={!tillNumber || !amount || isProcessing} onPress={handlePayment}>
              {isProcessing ? <ActivityIndicator color={theme.colors.onPrimary} /> : <Text style={[styles.primaryButtonText, { fontFamily: getFont("DMSans_700Bold") }]}>Make Payment</Text>}
            </TouchableOpacity>
          </View>
        </ScrollView>
  ...
  if (subScreen === "success") return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <View style={[styles.flex, styles.centered, { padding: 40 }]}>
        ...
        <TouchableOpacity style={[styles.primaryButton, { width: "100%", marginTop: 40 }]} onPress={() => { setAmount(""); setPhoneNumber(""); setBizNumber(""); setAccountNumber(""); setTillNumber(""); setSubScreen("main"); }}>
          <Text style={[styles.primaryButtonText, { fontFamily: getFont("DMSans_700Bold") }]}>Back to Payments</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollPadding}>
        ...
        <View style={styles.sendBalanceSection}>
          ...
          <View style={styles.percentBadge}>
            <Text style={[styles.percentText, { fontFamily: getFont("JetBrainsMono_500Medium") }]}>↗ +2.4%</Text>
          </View>
        </View>

        <View style={styles.serviceGrid}>
          <ServiceCard label="Send to M-Pesa" sub="Instant transfer" icon={<SendIcon color={theme.colors.onPrimary} size={22} />} active onPress={() => setSubScreen("mpesa_send")} />
          <ServiceCard label="Pay Bill" sub="Utilities & services" icon={<FileText color={theme.colors.primary} size={22} />} onPress={() => setSubScreen("paybill")} />
          <ServiceCard label="Buy Goods" sub="Till payments" icon={<ShoppingBag color={theme.colors.primary} size={22} />} onPress={() => setSubScreen("buy_goods")} />
        ...
        <View style={styles.payeeRow}>
          {[1, 2, 3, 4].map((i) => (
            <View key={i} style={styles.payeeAvatarContainer}>
...
function CustomTabBar({ state, descriptors, navigation }: any) {
  const iconMap: Record<string, React.ReactNode> = {
    Home: <Home size={24} />,
    Payment: <Wallet size={24} />,
    Visa: <CreditCard size={24} />,
  ...
  return (
    <View style={styles.bottomNav}>
      {state.routes.map((route: any, index: number) => {
        ...
        const isPayment = route.name === "Payment";
        ...
        const icon = React.cloneElement(iconMap[route.name] as React.ReactElement, {
          color: isFocused
            ? isPayment
        ...
        if (isPayment) {
          return (
            <TouchableOpacity key={route.key} style={styles.tabButton} onPress={onPress}>
              <View style={[styles.sendButtonActive, isFocused && { backgroundColor: theme.colors.primary }]}>
                {icon}
                {isFocused && <Text style={[styles.tabLabelActive]}>Send</Text>}
              </View>
            </TouchableOpacity>
          );
        }
        ...
        return (
          <TouchableOpacity key={route.key} style={styles.tabButton} onPress={onPress}>
            ...
            <Text style={[styles.tabLabel, isFocused && { color: theme.colors.primary }]}>
              {route.name}
            </Text>
          </TouchableOpacity>
        );
...
const styles = StyleSheet.create({
  ...
  tabIndicatorActive: { backgroundColor: "rgba(58, 223, 171, 0.15)" },
  tabLabel: { fontSize: 10, color: theme.colors.textSecondary },
  tabLabelActive: { fontSize: 10, color: theme.colors.onPrimary, marginTop: 2 },
  sendButtonActive: { width: 60, height: 60, borderRadius: 30, backgroundColor: "#1F2937", marginTop: -30, alignItems: "center", justifyContent: "center", borderWidth: 3, borderColor: "#050D17" },
});
